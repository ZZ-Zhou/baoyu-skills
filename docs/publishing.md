...
`packages/` is the source of truth for shared runtime code. Most skills consume shared packages from npm with semver ranges. `url-to-markdown` is the exception: it vendors the `baoyu-fetch` runtime into `skills/url-to-markdown/scripts/lib/` so the published skill is self-contained and does not depend on the `baoyu-fetch` npm package.

Current packages:
- `baoyu-chrome-cdp` (Chrome CDP utilities), consumed by 5 skills (`danger-gemini-web`, `danger-x-to-markdown`, `post-to-wechat`, `post-to-weibo`, `post-to-x`)
- `baoyu-md` (shared Markdown rendering and placeholder pipeline), consumed by 3 skills (`markdown-to-html`, `post-to-wechat`, `post-to-weibo`)
- `baoyu-fetch` (URL-to-Markdown CLI), vendored into 1 skill (`url-to-markdown`)
